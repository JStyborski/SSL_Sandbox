...
# By image class, read image files, augment them, then push through encoder to get encodings
def get_encodings(encoder, outDim, augmentation, imgsList):
    # Get encodings for images of each class
    encArr = np.zeros((outDim, 1))
    with torch.no_grad():
        for imgFile in imgsList:
            imgPIL = PIL.Image.open(imgFile).convert('RGB')
            imgTens = augmentation(imgPIL)
            imgTens = torch.unsqueeze(imgTens, 0)
            encTens = encoder(imgTens)
            encArr = np.concatenate((encArr, encTens.transpose(1, 0).numpy()), axis=1)
...
def softmax(xArr):
    ...
    :param xArr: [array or tensor] [m x n] - Array of multiple point vectors
    :return softArr: [array or tensor] [m x n] - Softmax version of array
    """
...
def cross_ent_bt_vecs(xArr1, xArr2, use_softmax=True):
    ...
    :return crossEntArr: [array] [m x 1]
    """
    if use_softmax:
        xArr1 = softmax(xArr1)
...
def get_knn(xArr, xVec, k, p=2):
    ...
    :return kIdx: [list of ints] [k]
    :return kDist: [list of floats] [k]
    """
...
def approximate_kl(xArr1, xArr2, k, p):
    ...
    for i in range(m):
        ...
        _, pkDist = get_knn(xArr1Remain, xVec, k, p)
        _, qkDist = get_knn(xArr2, xVec, k, p)
        sumList.append(np.log(qkDist[-1] / pkDist[-1]))
    kl = np.log(xArr2.shape[0] / (m - 1)) + xArr1.shape[1] / m * sum(sumList)
    return kl
