...
def save_chkpt(prefix, epoch, encArch, rnCifarMod, vitPPFreeze, prjArch, prjHidDim, prjBotDim, prjOutDim, prdHidDim, prdAlpha, prdEps, prdBeta, momEncBeta, applySG, decArch, model, optimizer):
    torch.save({'epoch': epoch, 'encArch': encArch, 'rnCifarMod': rnCifarMod, 'vitPPFreeze': vitPPFreeze, 'prjArch': prjArch, 'prjHidDim': prjHidDim,
                ...
                'momEncBeta': momEncBeta, 'applySG': applySG, 'decArch': decArch, 'stateDict': model.state_dict(),
...
def main():
    ...
    assert args.sslLossType in ['wince', 'bt', 'vicreg', 'mec', 'dino']
...
def main_worker(gpu, args):
    ...
    if args.trainLabels:
        if args.decArch is None:
            ...
            trainDataset = datasets.ImageFolder(args.trainRoot, CT.NTimesTransform_2Parts(args.nAugs, CT.t_cropflip(args.cropSize), CT.t_toPIL_highfilt(), CT.t_toPIL_pretrain()))
    else:
    ...
    model = SSLAE_Model.Base_Model(args.encArch, args.rnCifarMod, args.vitPPFreeze, args.prjArch, args.prjHidDim, args.prjBotDim, args.prjOutDim,
                                   args.prdHidDim, args.prdAlpha, args.prdEps, args.prdBeta, args.momEncBeta, args.applySG, args.decArch)
    ...
    if args.sslLossType == 'wince':
        ...
        sslLossFn = SSLAE_Loss.Barlow_Twins_Loss(args.symmetrizeLoss, args.btLam, args.btLossType, args.btNormType)
        ...
        sslLossFn = SSLAE_Loss.MEC_Loss(args.symmetrizeLoss, args.mecEd2, args.mecTaylorTerms)
    elif args.sslLossType == 'dino':
        sslLossFn = SSLAE_Loss.DINO_Loss(args.symmetrizeLoss, args.dinoCentMom, args.dinoTauS, args.dinoTauT)
    ...
    save_chkpt(args.ptPrefix, 0, args.encArch, args.rnCifarMod, args.vitPPFreeze, args.prjArch, args.prjHidDim, args.prjBotDim, args.prjOutDim,
               args.prdHidDim, args.prdAlpha, args.prdEps, args.prdBeta, args.momEncBeta, args.applySG, args.decArch, model, optimizer)
    timeStart = time.time()
    ...
    for epoch in range(args.startEpoch, args.nEpochs + 1):
        ...
        for batchI, batch in enumerate(trainDataLoader):
            ...
            if any(args.useAdvList):
                _, advTensList = FGSM_PGD.ssl_pgd(model, sslLossFn, augList, args.useAdvList, args.gatherTensors, args.advAlpha, args.advEps,
            ...
            for augTens in augList:
                ...
                if args.gatherTensors:
                    ...
                    xd = torch.cat(MF.FullGatherLayer.apply(xd.contiguous()), dim=0) if args.decArch is not None else xd
            ...
            # Calculate loss
            sslLossVal = sslLossFn(outList)
            if args.decArch is not None:
                aeLossVal = aeLossFn(recList, srcList)
