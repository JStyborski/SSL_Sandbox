...
def ComputeThresholdByTopPercentage(attributions, percentage=60, plot_distribution=True):
    ...
    # For percentage equal to 100, this should in theory return the lowest value as the threshold.
    # However, due to precision errors in numpy's cumsum, the last value won't sum to 100%.
    # Thus, in this special case, we force the threshold to equal the min value.
    if percentage == 100:
    ...
    # 0th percentile values correspond to the biggest attributions, 100th percentile values correspond to the smallest
...
def LinearTransform(attributions, clip_above_percentile=99.9, clip_below_percentile=70.0, low=0.2, plot_distribution=False):
    ...
    # Transform the attributions by a linear function f(x) = a*x + b such that f(m) = 1.0 and f(e) = low
    # Derivation:
...
def Outlines(attributions, percentage=90, connected_component_structure=np.ones((3, 3)), plot_distribution=True):

    # Binarize the attributions mask if not already.
    attributions = attributions > 0.001
...
def Visualize(attributions, image, positive_channel=(0, 255, 0), negative_channel=(255, 0, 0), polarity='positive',
              clip_above_percentile=99.9, clip_below_percentile=0, morphological_cleanup=False, structure=np.ones((3, 3)),
              outlines=False, outlines_component_percentage=90, overlay=True, plot_distribution=False):

    if polarity == 'both':
        # If polarity is 'both', then run Visualize twice with positive and negative settings, then combine results

        pos_attributions = Visualize(
            attributions, image, positive_channel=positive_channel, negative_channel=negative_channel, polarity='positive',
            ...
            overlay=False, plot_distribution=plot_distribution)

        neg_attributions = Visualize(
            attributions, image, positive_channel=positive_channel, negative_channel=negative_channel, polarity='negative',
            clip_above_percentile=clip_above_percentile, clip_below_percentile=clip_below_percentile,
            morphological_cleanup=morphological_cleanup, outlines=outlines,
            outlines_component_percentage=outlines_component_percentage,
            overlay=False, plot_distribution=plot_distribution)
        ...
        if overlay:
            attributions = np.clip(0.5 * image + 0.5 * attributions, 0, 255)
    ...
    # Convert to grayscale (HxWx3 --> HxW)
    attributions = np.average(attributions, axis=2)

    # Scale and clip to range
    attributions = LinearTransform(attributions, clip_above_percentile, clip_below_percentile, 0.0, plot_distribution=plot_distribution)

    # Both unused by default - I don't know what they do
    if morphological_cleanup:
        attributions = MorphologicalCleanup(attributions, structure=structure)
    if outlines:
        attributions = Outlines(attributions, percentage=outlines_component_percentage, plot_distribution=plot_distribution)
